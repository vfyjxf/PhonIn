...
/**
 * Loads the bundled dataset (raw {@code *-char.tsv} / {@code *-word.tsv}) from the classpath into
 * {@link PhoneticSystem} instances. The system objects own their data afterwards, so the matcher
 * never goes through a lookup layer — it reads {@link PhoneticSystem#charEntry(int)} directly.
 *
 * <p>By default nothing is loaded. Use {@link #loadFromClasspath()} to load all bundled systems, or
 * {@link #load(PhoneticSystem...)} / {@link #loadForOptions(Options)} to load only what a call
 * needs. Re-loading is a no-op for already-loaded systems.
 */
public final class PhonInData {

    private static final PhoneticSystem[] ALL_SYSTEMS = {
    ...
    /** Load every bundled system. */
    public static PhonInData loadFromClasspath() {
        PhonInData d = empty();
        d.loadAll();
        return d;
    }
    ...
    /** Load all bundled systems. */
    public synchronized void loadAll() {
        load(ALL_SYSTEMS);
    }

    /** Load one or more bundled systems. Already-loaded systems are skipped. */
    ...
    public void loadForOptions(Options options) {
        if (options.system() != null) {
            ...
            if (known.isEmpty()) {
                // All known systems already loaded, or router declared none.
                // Custom router with no declared systems: we cannot know what it will return,
                // so fall back to loading all bundled systems to avoid silent misses.
                if (loaded.size() < ALL_SYSTEMS.length) {
                    loadAll();
                }
            } else {
    ...
    private void loadSystem(PhoneticSystem system) {
        ...
        try (InputStream in = openResource(charPath)) {
            if (in == null) return; // system absent from the bundled dataset; do not clear
            system.clearData();
            loadCharTable(system, in);
        } catch (IOException e) {
            throw new RuntimeException("Failed to load " + charPath, e);
        }
        // Word tables are loaded into PhoneticSystem.words() but are not consumed by the
        // matcher (Phase 2C). Skip them to save startup memory.
    }
