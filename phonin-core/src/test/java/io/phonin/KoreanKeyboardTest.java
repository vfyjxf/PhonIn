...
class KoreanKeyboardTest {
    ...
    void koreanOptionMatchesBulsik2() {
        ...
        assertThat(phonIn.contains("한", "gks", SystemOptions.korean())).isTrue();
        assertThat(phonIn.contains("한", "han", SystemOptions.korean())).isFalse(); // not romaji
        assertThat(phonIn.contains("가", "rk", SystemOptions.korean())).isTrue();
        assertThat(phonIn.contains("중", "wnd", SystemOptions.korean())).isTrue();
    }

    @Test
    void koreanChoseongOptionMatchesInitialConsonants() {
        // choseong: 한→g, 글→r, so "gr" matches 한글
        assertThat(phonIn.contains("한", "g", SystemOptions.koreanChoseong())).isTrue();
        assertThat(phonIn.contains("한", "gks", SystemOptions.koreanChoseong()))
                .isFalse(); // full keys don't match
        // Multi-char: 한글 with choseong "gr"
        assertThat(phonIn.contains("한글", "gr", SystemOptions.koreanChoseong())).isTrue();
    }
    ...
    void koreanPartialMatch() {
        ...
        assertThat(phonIn.contains("한", "g", SystemOptions.korean())).isTrue();
        assertThat(phonIn.contains("한", "gk", SystemOptions.korean())).isTrue();
    }
