 ...
 * no {@link io.phonin.match.Accelerator} cache. Each name gets a fresh per-call {@link
 * MatchContext}, so this is the <em>only</em> built-in searcher that supports {@link
 * io.phonin.PolyphoneMode#PRECISE}: the position-aware {@link MatchContext#charNode(int[], int)}
 * path requires a per-name matching frame, which the accelerator's shared per-codepoint cache
 * cannot provide.
 *
