 ...
 * cache instead of {@link io.phonin.match.Accelerator}. Search descends the trie guided by cached
 * {@link CharNode#match}; flat {@link NDense} leaves test the remaining name suffix via a
...
public final class PreciseSearcher<T> implements Searcher<T> {
    ...
    // ---- CharNode interning: (codepoint, forcedSyllable) → phonetic key → CharNode ----
    ...
    // ---- name pool: phonetic keys concatenated with 0 sentinel ----
    ...
    // ---- trie nodes (mirror TreeSearcher's NDense/NSlice/NMap) -----------
